...
from .forms import PostForm, AttachmentForm, AttachmentFormSet
...
def post_create(request):
    ...
    if form.is_valid():
        ...
        if (post.label == "End"):
            journey.on_going = False
        else:
            journey.on_going = True
        journey.save()
...
def post_unlike(request, pk):
    ...
    if post.likes.filter(created_by=request.user):
        ...
        return JsonResponse({'message': 'post already unliked'})
